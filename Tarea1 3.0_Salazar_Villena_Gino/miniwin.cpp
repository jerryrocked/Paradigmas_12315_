...
inline void _elip(float x_cen, float y_cen, float radio_men, float radio_may) {
   BeginPath(hDCMem);
   Arc(hDCMem, int(x_cen - radio_men), int(y_cen - radio_may),
               int(x_cen + radio_men), int(y_cen + radio_may),
               int(x_cen - radio_men), int(y_cen - radio_may),
               int(x_cen - radio_men), int(y_cen - radio_may));
   EndPath(hDCMem);
}
void elipse_lleno(float x_cen, float y_cen, float radio_men, float radio_may) {
   HBRUSH hBrush = CreateSolidBrush(_color);
   HGDIOBJ orig = SelectObject(hDCMem, hBrush);
   _elip(x_cen, y_cen, radio_men, radio_may);
   FillPath(hDCMem);
   SelectObject(hDCMem, orig);
   DeleteObject(hBrush);
}
